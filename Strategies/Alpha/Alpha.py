...
class Alpha(strategy):
    ...
    def add_features(self, data, params):
        ...
        results = pool.map(add_fisher, inputs)
        pool.clear()
        for result in results:
            data[result.columns[-1]] = result.filter(regex='Fisher.*')
        self.data = data
        return self.data
    ...
    def create_signals(self, df_input, *params):
        ...
        df["fisher"] = df[f"Fisher{lookback}"]
        df = df.loc[(df.fisher != 0)]
        df["fisher_lag"] = df.fisher.shift(1)
        df["lb"] = lower_bound
        df["ub"] = upper_bound
        df.dropna()
        ...
        buy_mask = (df["fisher"] > df["lb"]) & (df["fisher_lag"] < df["lb"])
        sell_mask = ((df["fisher"] < df["ub"]) & (df["fisher_lag"] > df["ub"]))
        ...
        return df1[["Datetime", "signal", "lb", "ub", "fisher"]]
    ...
    def plotting_function(self, df, save_to=None):
        ...
        plt.plot(df['Datetime'], df['fisher'], color='black', label='Fisher')

        plt.plot(df['Datetime'], df['lb'], color='green', label='Lower Bound')
        plt.plot(df['Datetime'], df['ub'], color='red', label='Upper Bound')

        plt.plot(df.loc[buy_plot_mask]['Datetime'].shift(1), df.loc[buy_plot_mask]['fisher'].shift(1), r'^',
                 ms=15,
                 label="Entry Signal", color='green', markeredgecolor='k', markeredgewidth=1)
        plt.plot(df.loc[sell_plot_mask]['Datetime'].shift(1), df.loc[sell_plot_mask]['fisher'].shift(1), r'^',
                 ms=15,
                 label="Exit Signal", color='red', markeredgecolor='k', markeredgewidth=1)
        plt.title('Strategy Backtest')
        plt.legend(loc=0)
    ...
    def get_optimization_params():
        feature_space = [config.fisher_lookbacks]
