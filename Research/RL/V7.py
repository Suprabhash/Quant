...
def train_q_learning(train, state_lookback, model, alpha, epsilon, gamma, episodes, all_actions, metric, features, plot=True):
    ...
    for ii in tqdm(range(episodes)):
        ...
        next_q_all_states = []
        previous_portfolio_percentage = 0
        # q = model.predict(arr)
        actions = model.predict(arr)
        ...
        for i in range(1,len(train_data)):
            ...
            states.append(arr[i])
            current_q_all_states.append(actions[i])
            ...
            portfolio_percentage = action
            # outcomes_history.append(outcome + max_units)

            balance = balance - (portfolio_percentage - previous_portfolio_percentage) * current_adj_close
            position_value = portfolio_percentage * current_adj_close
            ...
            net_worth = balance + position_value
            equity_curve.append(net_worth)
            previous_portfolio_percentage = portfolio_percentage
            try:
                next_q_all_states.append(actions[i+1])
            except:
                break
...
if __name__ == "__main__":
    ...
    all_actions = ["action"]#{0: 'neutral', 1: 'long'}
    model = get_model(len(features)*state_lookback, len(all_actions), num_dense_layers, len(features)*state_lookback*num_dense_layers_by_num_features)
