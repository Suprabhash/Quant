    ...
    for ii in tqdm(range(episodes)):
        ...
        for i in range(1,len(train_data)):
            ...
            if not in_position:
                ...
                todays_market_return = ((current_adj_close - last_day_adj_close) / last_day_adj_close)
                if action == 1:  # HOLD LONG
                    position_value = bet_bought * (1.0 + market_return)
                    if metric == "absolute":
                        rewards.append(todays_market_return*last_day_adj_close)
                    else:
                        rewards.append(todays_market_return)
                else:  # CLOSE LONG
