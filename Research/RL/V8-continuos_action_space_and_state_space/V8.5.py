...
def train_q_learning(train, state_lookback, model, alpha, epsilon, gamma, episodes, all_actions, metric, features,
    ...
    for ii in tqdm(range(episodes)):
        ...
        if episode_return>best_episode_return:
            ...
            # best_model = model
            model.save("best_performing_model_so_far_sinx")
...
if __name__ == "__main__":
    ...
    if tune:
        ...
        episodes = save["episodes"] = 20
        number_of_random_samples = save["number_of_random_samples"]  = 20
    ...
    #                                                        episodes, all_actions, metric, features,number_of_random_samples, plot=False)

    best_model = load_model("best_performing_model_so_far_sinx")
    number_of_random_samples = 20
    # fig, score = eval_q_learning(train_df, best_model, state_lookback, all_actions, number_of_random_samples, metric,
    #                              features, save=True)
    fig, score = eval_q_learning(test_df, best_model, state_lookback,all_actions,number_of_random_samples, metric, features, save=True)
