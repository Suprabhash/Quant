...
def get_model(num_features, num_outcomes, num_dense_layers, neurons_per_layer):
    model = Sequential()
    model.add(InputLayer(batch_input_shape=(1, num_features)))
    for i in range(num_dense_layers):
        model.add(Dense(neurons_per_layer, activation='relu'))
    model.add(Dense(num_outcomes, activation='linear'))
    model.compile(loss='mse', optimizer='adam', metrics=['mae'])
    return model

def plot_performance(df, prices, features, outcomes_history, equity_curve, save=False):
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(15, 7))
    # fig, (ax1, ax3) = plt.subplots(2, 1, figsize=(15, 7))
    ax1.plot(prices, label='Close')
    ax1_copy = ax1.twinx()
    ax1_copy.plot([i-max_units for i in outcomes_history], label='Actions')
    ax2.plot([i-max_units for i in outcomes_history], label='Actions')
    ax2_copy = ax2.twinx()
    for feature in [feature["feature"] for feature in features]:
        ax2_copy.plot(df[feature].values[:len(outcomes_history)], label=feature, color='green', ls='dotted')
    ax2_copy.axhline(0.0, ls='--', color='grey')
    ax3.plot(equity_curve, label='Net worth')
    ax3.plot([price for price in prices], label='Benchmark')
    ax1.legend()
    # ax2.legend()
    ax3.legend()
    if type(save)==str:
...
def train_q_learning(train, state_lookback, model, alpha, epsilon, gamma, episodes, all_outcomes, metric, features, plot=True):
    ...
    for ii in tqdm(range(episodes)):
        ...
        previous_outcome = 0
        previous_adj_close = train_data.iloc[0]["Close"]
        update_qtable = True
        q = model.predict(arr)
        ...
        for i in range(1,len(train_data)):
            ...
            outcome = outcome_priority[0] - max_units
            outcomes_history.append(outcome+max_units)
            ...
            if equity_curve[-1] <= losing_threshold*initial_capital:
                # update_qtable=False
                break
...
def eval_q_learning(test_data, model, state_lookback, metric, features, save=False):
    ...
    current_q_all_states = []
    previous_outcome = 0
    benchmark_units = round(initial_capital / test_data.iloc[0]["Close"])
    q = model.predict(arr)
    outcomes = (-1*q).argsort()
    ...
    return plot_performance(test_data, benchmark_holding, features, outcomes_history, equity_curve,
                            save=True), portfolio_return
    # return "hi",portfolio_return
...
def hyperparametric_tuning_optuna(run):
    def objective(trial):
        ...
        state_lookback = 1#int(params["state_lookback"])
        ...
        all_outcomes = {k: f"your current units must be {k}" for k in range(-max_units, max_units + 1)}
        ...
        model, returns_vs_episodes, weights = train_q_learning(train_df, state_lookback, model, alpha, epsilon, gamma, episodes,all_outcomes, metric, features, plot=False)
...
if __name__ == "__main__":
    ...
    save["ExperimentName"] = f"Run {datetime.now().strftime('%H:%M:%S')}: Hyperparameteric tuning for short and long 5 units sin wave"
    ############################################################################################################################################
    # Experiment Features
    features = [
    ...
    #Experiment params
    ticker = save["ticker"] = 'sinx'
    tune = False
    if tune:
        tuned_params = hyperparametric_tuning_optuna(run)
        state_lookback = save["state_lookback"] = 1#int(tuned_params["state_lookback"])
    ...
    # all_actions = {k:(f"sell {-k} units" if k<0 else f"hold" if k ==0 else f"buy {k} units") for k in range(-max_units,max_units+1)}
    all_outcomes = {k:f"your current units must be {k}" for k in range(-max_units,max_units+1)}
    model = get_model(len(features)*state_lookback, len(all_outcomes), num_dense_layers, len(features)*state_lookback*num_dense_layers_by_num_features)
    # visualizer(model, format='png', view=True)

    model, returns_vs_episodes, weights = train_q_learning(train_df, state_lookback, model, alpha, epsilon, gamma, episodes, all_outcomes, metric, features,plot=True)
    model.set_weights(weights)
