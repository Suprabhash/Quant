...
def train_q_learning(train, state_lookback, model, alpha, epsilon, gamma, episodes, all_actions, metric, features,
    ...
    for ii in tqdm(range(episodes)):
        ...
        log_list.append((pd.DataFrame(q.reshape(int(len(q)/10),10)),arr_fit_X,arr_fit_Y,states, actions_history, rewards))
    ...
    log_list.append(returns_vs_episodes)
    time_now = datetime.now().strftime("%Y_%m_%d_%HH_%MM_%SS")
    folder_name = os.getcwd() + "\\logs_train"
    with open(f"{folder_name}\\logging_train_{time_now}.pkl","wb") as file:
