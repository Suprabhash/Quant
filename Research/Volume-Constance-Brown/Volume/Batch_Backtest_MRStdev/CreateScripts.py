...
if __name__ == '__main__':
    ...
    with open(f'NSEI_Volume_MeanRevertingStdev_Backtest_Azure_temp_og.pkl','rb') as file:
        temp_og = pickle.load(file)

    # temp_og = pd.read_csv(
    #     "https://raw.githubusercontent.com/AcsysAlgo/VolumeData/main/NSEI_Volume_MeanRevertingStdev_Backtest_Azure_temp_og.csv")
    ...
    shutil.copyfile("NSEI_Volume_MeanRevertingStdev_Backtest_Azure_temp_og.pkl", "InputFiles/NSEI_Volume_MeanRevertingStdev_Backtest_Azure_temp_og.pkl")
    ...
    for train_months in [24,48,96]:
        for date_i in range(len(dates) - (int(train_months / 3) + 1)):
            ...
            if not TEST:
                with open('NSEI_Volume_MRStdev_Backtest_Azure_Template.py', "rt") as fin:
                ...
                with open('NSEI_Volume_MRStdev_Backtest_Azure_Tester.py', "rt") as fin:
                    with open(f"InputFiles/{ticker}_TrainYrs_{int(train_months / 12)}_All_Strategies_{date_i}.py", "wt") as fout:
                        for line in fin:
                            fout.write(line.replace("ticker_inp", f"'{ticker}'").replace("date_i_inp", f"{date_i}").replace("train_months_inp", f"{train_months}"))
            ...
            def zipdir(path, ziph):
                ...
                for root, dirs, files in os.walk(path):
                    ...
                    for file in files:
                        if (file==f"{ticker}_TrainYrs_{int(train_months / 12)}_All_Strategies_{date_i}.py") | (file=="NSEI_Volume_MeanRevertingStdev_Backtest_Azure_temp_og.pkl"):
