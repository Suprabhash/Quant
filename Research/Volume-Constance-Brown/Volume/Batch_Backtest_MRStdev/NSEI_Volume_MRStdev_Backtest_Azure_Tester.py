...
def backtest_mr_stdev_for_avg_sortino(input):
    ...
    try:
        ...
        return {"levelb": input[1], "lookbackb": input[2], "band_buy": input[3], "xtp": input[4], "ktp": input[5],
                "leveltp": input[6], "lookbacktp": input[7], "xsl": input[8], "ksl": input[9], "levelsl": input[10],
                "lookbacksl": input[11], "avg_sortino_of_trades": strat.daywise_performance['avg_sortino_of_trades']}
...
if __name__ == '__main__':
    ...
    with open(f'{ticker}_TrainYrs_{int(train_months / 12)}_All_Strategies_{date_i}.pkl',
              'wb') as file:
        pickle.dump(BFO_vol_mr_stdev_strategy(temp), file)
