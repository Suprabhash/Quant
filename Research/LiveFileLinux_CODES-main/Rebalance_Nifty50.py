...
if __name__ == '__main__':
    ...
    index = ".BSESN"
    with open(f'NSE_Constituents.pkl', 'rb') as file:
        constituents = pickle.load(file)
    dates = constituents.Date
    ...
    latest = get_weights_stocks_live(constituents, top_nassets, recalibrating_months, training_period, dates_recalibrating,
                                data_inp, save=False)[0]

    with open(f'Z:/Algo/Live_emailer_updates/files_for_rebalance/BSE_RecalibPeriod_latest.pkl', 'wb') as file:
