    ...
    while True:
        if ((datetime.now().hour==run_hour) & (datetime.now().minute==run_minute) & (datetime.now().second==00)) or running_for_the_first_time:
            if (datetime.today().isoweekday() < 6):
                ...
                #     pass
                # else:
                SendMail(subject_text_add, text_restricted,text_midcap, text_gold, text, printdf_midcap, printdf_gold, pd.concat([assetsb[-1][["Ticker", "Accelerating Momentum"]].set_index("Ticker"), pd.DataFrame([unit_ticker]).transpose().rename(columns={0:"Units"})], axis=1), [f"Performance{ticker_midcap}.jpg", f"Performance{ticker_gold}.jpg", f"PerformanceAMMidcapA2.jpg"])
