...
def select_all_strategies(train_monthsf, datesf, temp_ogf, ticker, save=True):
    ...
    if len(inputs) == 0:
        ...
        res_test_update[0] = pd.concat([res_test_update[0], results[0][1].reset_index().drop(['index'], axis=1)],
                                       axis=0)

    if save == True:

        if not os.path.exists(f'Z:/Algo/Live_emailer_updates/zip_files_for_recalibration/{ticker}/SelectedStrategies'):
            os.makedirs(f'Z:/Algo/Live_emailer_updates/zip_files_for_recalibration/{ticker}/SelectedStrategies')

        res_test = [res_test_update[-1]]
        with open(f'Z:/Algo/Live_emailer_updates/zip_files_for_recalibration/{ticker}/SelectedStrategies/{ticker}_TrainYrs_{int(train_monthsf / 12)}_All_Strategies.pkl',
                  'wb') as file:
...
def select_strategies_from_corr_filter(res_testf2, res_testf4, res_testf8, datesf, temp_ogf, num_opt_periodsf,
    ...
    if save == True:
        ...
        with open(f'Z:/Algo/Live_emailer_updates/zip_files_for_recalibration/{ticker}/SelectedStrategies/{ticker}_OptPeriods_{num_opt_periodsf}_Selected_Strategies_ss.pkl',
                  'wb') as file:
            pickle.dump(ss_test, file)
        with open(f'Z:/Algo/Live_emailer_updates/zip_files_for_recalibration/{ticker}/SelectedStrategies/{ticker}_OptPeriods_{num_opt_periodsf}_Selected_Strategies_res.pkl',
                  'wb') as file:
            pickle.dump(res_test, file)
...
if __name__ == '__main__':
    ...
    if ((pd.to_datetime(date.today()+timedelta(1)) in dates_all_ss) or (pd.to_datetime(date.today()) in dates_all_ss)):
        ...
        ss_test_imp, res_test_imp = select_strategies_from_corr_filter(res_test2, res_test4, res_test8, dates_ss,
                                                                       temp_og,
        ...
        if not os.path.exists(f'Z:/Algo/Live_emailer_updates/zip_files_for_recalibration/{ticker}/weights'):
            os.makedirs(f'Z:/Algo/Live_emailer_updates/zip_files_for_recalibration/{ticker}/weights')

        with open(f'Z:/Algo/Live_emailer_updates/zip_files_for_recalibration/{ticker}/weights/Results_Ticker{ticker}_LP{number_of_optimization_periods}_Recal{recalib_months}_NS{num_strategies}_M{metric}.pkl',
                'wb') as file:
