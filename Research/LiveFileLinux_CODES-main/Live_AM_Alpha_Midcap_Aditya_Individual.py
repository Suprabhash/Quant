...
def SendMail(subject_text_add,rebalance_recalibration_df,text_restricted,text_midcap, text_gold, text, printdf_midcap, printdf_gold, assets, ImgFileNameList, csv_file_path):
    msg = MIMEMultipart()
...
def execute_gold(ticker, number_of_optimization_periods,recalib_months,num_strategies,metric):
    ...
    temp_res.to_csv("Gold_Alpha_Midcap_Naive.csv")
    ...
    while True:
        if ((datetime.now().hour==run_hour) & (datetime.now().minute==run_minute) & (datetime.now().second==00)) or running_for_the_first_time:
            if (datetime.today().isoweekday() < 6):
                ...
                text_restricted = "[ADITYA INDIVIDUAL]"+ "\n"+"Restricted List is : [PGHH, PFIZER]" + "\n"
                ...
                temp_res.to_csv("MidcapNaive.csv")

                azure_update_status_message = update_nifty_midcap_daily("MidcapNaive.csv")
                azure_update_status_message = azure_update_status_message + update_gold_daily("Gold_Alpha_Midcap_Naive.csv")
                ...
                plt.legend(loc=0)
                plt.tight_layout()
                plt.savefig("PerformanceAMMidcapAI.jpg")
                plt.clf()
                ...
                dates_recalibrate.append({"Rebalancing Date": "Accelerating Momentum", "Date": dates_rebalancing[-2] + relativedelta(months = rebalancing_months)})


                if running_for_the_first_time == True:
                    pass
                else:
                    SendMail(subject_text_add, pd.DataFrame(dates_recalibrate).set_index("Rebalancing Date").sort_values(by="Date", ascending=True), text_restricted,text_midcap, text_gold, text, printdf_midcap, printdf_gold, pd.concat([assetsb[-1][["Ticker", "Accelerating Momentum"]].set_index("Ticker"), pd.DataFrame([unit_ticker]).transpose().rename(columns={0:"Units"})], axis=1), [f"Performance{ticker_midcap}.jpg", f"Performance{ticker_gold}.jpg", f"PerformanceAMMidcapAI.jpg"], "MidcapNaive.csv")
