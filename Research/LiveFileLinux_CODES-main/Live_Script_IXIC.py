...
def get_data(ticker):
    ...
    temp_og_usdinr = gd("USD/INR", "investpy", "india").set_index("Date")["Close"].to_frame().rename(
        columns={"Close": "USDINR"})
    temp_og = pd.merge(temp_og, temp_og_usdinr, on="Date", how="left")
    temp_og["INRClose"] = temp_og["Close"] * temp_og["USDINR"]
...
def SendMail(ticker, text, printdf, ImgFileName, csv_file_path):
    msg = MIMEMultipart()
    ...
    while True:
        if  ((datetime.now().hour==run_hour) & (datetime.now().minute==run_minute) & (datetime.now().second==00)) or running_for_the_first_time:
            if (datetime.today().isoweekday() < 6):
                ...
                temp_res = pd.concat([results_final, temp_og[["Date", "INRClose", "USDINR"]].set_index("Date")], axis=1,join="inner")

                initial_amount = 2553919
                ...
                for i in range(len(temp_res)):
                    ...
                    if ((temp_res["signal"].iloc[i] == 1) & (temp_res["signal"].shift(1).fillna(0).iloc[i] == 0)):
                        units_equity = equity_allocation / temp_res.iloc[i]["INRClose"]

                    if ((temp_res["signal"].iloc[i] == 0) & (temp_res["signal"].shift(1).fillna(1).iloc[i] == 1)):
                        units_equity = 0

                    if signal == 1:
                        equity_allocation = units_equity * temp_res.iloc[i]["INRClose"]
                    if i > 0:
                        cash_allocation = cash_allocation * (
                                    1 + 6 / 25200 + ((temp_res['USDINR'].iloc[i] / temp_res['USDINR'].iloc[i - 1]) - 1))
                    else:
                ...
                temp_res["Market_Return"] = (initial_amount / temp_res.iloc[0]["INRClose"]) * temp_res["INRClose"]
                temp_res.reset_index(inplace=True)

                temp_res.to_csv("^IXIC.csv")
                azure_update_status_message = update_ixic_daily("^IXIC.csv")
                ...
                text = f"Signal at : {str(today_time_close)[:11]} : {signal_print(temp_res.iloc[-1]['Signal_backtest'])}" + "\n" f"Signal at Yesterday Close: {signal_print(temp_res.iloc[-2]['Signal_backtest'])}" + "\n" f"Signal at Day before Yesterday Close: {signal_print(temp_res.iloc[-3]['Signal_backtest'])}" + "\n" + "\n" + "Overall Performance:" + "\n" + f"Portfolio Value: {np.round(temp_res.iloc[-1]['Strategy_Return'], 2)/10000000} Cr" + "\n" f"Units of Motilal Oswal NASDAQ 100 ETF: {temp_res.iloc[-1]['Strategy_Return']/get_data_investpy(symbol='N100', country='india', from_date='01/01/2007',to_date=(date.today()+timedelta(1)).strftime('%d/%m/%Y'))['Close'].iloc[-1]}"+"\n" + "\n" + text

                if running_for_the_first_time == True:
                    pass
                else:
                    SendMail(ticker, text, results_backtest[-1][1], f"Performance{ticker}.jpg", "^IXIC.csv")
            ...
            #     ss_test_imp, res_test_imp = select_strategies_from_corr_filter(res_test2,res_test4,0, dates_ss, temp_og, number_of_optimization_periods,10, ticker, save=True)
            #
            #     res_test = []
            #     ss_test = []
            #     dates = []
