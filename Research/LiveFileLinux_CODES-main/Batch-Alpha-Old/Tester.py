...
def get_strategies_brute_force(inp):
    ...
    for f_look in range(210, 250, 20):  #50, 410, 20
        max_metric = 0
        for lb in np.round(np.arange(-1, 0.5, 0.25), decimals=1):
            for ub in np.round(np.arange(-1, 0.5, 0.25), decimals=1):#-7, 7, 0.25
...
def select_strategies_from_corr_filter(res_testf2,res_testf4,res_testf6, datesf, temp_ogf, num_opt_periodsf,num_strategiesf, ticker, save=True):
    ...
    for i in range(len(res_total)):
        ...
        temp =  res_total_all[i].drop_duplicates(subset = ['Lookback', 'Low Bound', 'High Bound'],keep = 'first').reset_index(drop = True)
        temp["AvgWinLoss"] = np.nan
        for k in range(len(temp)):
            metrics = np.array(list(res_total_all[i][(res_total_all[i]["Lookback"]==temp.iloc[k]["Lookback"])&(res_total_all[i]["Low Bound"]==temp.iloc[k]["Low Bound"])&(res_total_all[i]["High Bound"]==temp.iloc[k]["High Bound"])]["AvgWinLoss"]))
...
if __name__ == '__main__':
    ...
    # ( ALL FUNCTIONS UNDER THIS PHASE HAS BEEN DEBUGGED AND CHECKED -11/1/2022 )
    res = []
    for number_of_optimization_periods in [1]:
        for recalib_months in [3]:
            for num_strategies in [1]:
                for metric in ['rolling_sharpe']:
                    try:
