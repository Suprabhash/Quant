...
def get_data(ticker, api):
    ...
    print(f"Starting Date: {temp_og['Date'].iloc[0]}")
    ...
    # temp_og[["Close", "High", "Low", "Open", "Volume"]] = temp_og[["Close", "High", "Low", "Open", "Volume"]].astype(
    #     float)
    # temp_og = add_fisher(temp_og)
...
def optimize_weights_and_backtest(input):
    ...
    def get_equity_curve(*args):
        ...
        ec = test_strategy.signal_performance(10000, interest_inp)
    ...
    if len(ss_test[date_i]) > 0:
        ...
        # Structuring the selected strategies according to number
        strategies = ["Strategy" + str(i) for i in range(1, len(res) + 1)]
        params = selected_params(strategies, res)

        # Split the data into train and test set. Use the given parameters to calculate - strategy returns and strategy signals
        ...
        # The below code is a MCMC method based optimization to choose the weights(importance) to be assigned to the different strategies available to us. It takes in a metric as input to
        # optimize the weights while maximising that metric
        ...
        if len(guess)>1:
            ...
            # Store the obtained weights
            weights = []
            for weight in mc.analyse_results(rs, top_n=1)[0][0]:
                weights.append(weight)
            weights = pd.DataFrame(weights)
            # Normalizing the weights values so that it may all add upto 1
        ...
        # Testing the params on the test-set now WITHOUT the weights
        _, signal = top_n_strat_params_rolling(temp, res, to_train=False, num_of_strat=len(selected_strategies),
                                               split_date=str(dates[date_i]))

        # The final signal is now created with the weights
    ...
    test_strategy = FISHER_MCMC(inp.drop(['signal', 'Date'], axis=1), inp['signal'])
    _ = test_strategy.generate_signals()
    tt = test_strategy.signal_performance(1, interest_inp)
    return date_i, tt.set_index(pd.to_datetime(tt["Date"])).drop(columns="Date"), weights

def get_strategies_brute_force(inp):
    def get_equity_curve_embeddings(*args):      # Given the data, this fn creates the fisher signals and generates signals - 0s and 1s based on which signal performance is then done
        f_look = args[0]
    ...
    temp_og = inp[2]
    train_months = inp[3]
    # Data is filtered on either 24, 48 or 96 months.
    ...
    # Main brute force logic here which calculates avgwinloss of different strategies and stores only the better performing ones
    for f_look in range(50, 410, 20):  #50, 410, 20
        max_metric = 0
        for lb in np.round(np.arange(-7, 7, 0.25), decimals=1):
            for ub in np.round(np.arange(-7, 7, 0.25), decimals=1):#-7, 7, 0.25
...
def top_n_strat_params_rolling(temp, res, to_train, num_of_strat, split_date):
    if len(res)>0:
        for i in range(num_of_strat):
            ...
            dummy = test_strategy.signal_performance(10000, interest_inp)
            if i==0:
...
def select_all_strategies(train_monthsf, datesf, temp_ogf, ticker, save=True):
    # We use a brute force method to test for different combinations of lower bound, upper bound and Fisher lookback values. The results are arranged by 'AvgWinLoss' criteria
    inputs =[]
    # Each period length is created separately so that it is easier to feed to  multiprocessing pools
    ...
    # Storing the results by shifting the indexes, so that on any particular index we have results of backtested 2,4,8 years combined together at a later stage in the code.
    for i in range(len(results)):
        shift = results[i][0] + int((train_monthsf -24)/3)
        res_test[shift] = pd.concat([res_test[shift], results[i][1].reset_index().drop(['index'], axis=1)], axis=0)
...
def select_strategies_from_corr_filter(res_testf2,res_testf4,res_testf6, datesf, temp_ogf, num_opt_periodsf,num_strategiesf, ticker, save=True):
    # From the previous selected strategies for different periods - 24,48,96, we merge them all together if needed and then weed out the correlated strategies
    train_monthsf = 24  #minimum optimization lookback
    res_total = [None]*(len(datesf)-(int(train_monthsf/3)+1))
    # Here the different lengths of results are combined so that for ONE date, we have strategies selected separately for 24,48 and 96 months
    for i in range(len(datesf)-(int(train_monthsf/3)+1)):
        ...
        if num_opt_periodsf==3:
            res_total[i] = pd.concat([res_testf2[i],res_testf4[i],res_testf6[i]], axis=0)
        res_total[i] = res_total[i].reset_index().drop(['index'], axis=1)

    ###
    res_total_all = [pd.DataFrame(
        columns=["Lookback", "Low Bound", "High Bound", "AvgWinLoss", "Optimization_Years"])] * (len(res_total))
    lookback = 8
    for i in range(len(res_total)):
        if i >= lookback - 1:
            for j in range(i - (lookback - 1), i + 1):
                res_total_all[i] = pd.concat([res_total_all[i], res_total[j]])
        else:
            for j in range(0, i + 1):
                res_total_all[i] = pd.concat([res_total_all[i], res_total[j]])
        res_total_all[i] = res_total_all[i].sort_values(by="AvgWinLoss", ascending=False)
        res_total_all[i] = res_total_all[i].reset_index(drop=True)
        temp = res_total_all[i].drop_duplicates(subset=['Lookback', 'Low Bound', 'High Bound'],
                                                keep='first').reset_index(drop=True)
        temp["AvgWinLoss"] = np.nan
        for k in range(len(temp)):
            metrics = np.array(list(res_total_all[i][(res_total_all[i]["Lookback"] == temp.iloc[k]["Lookback"]) & (
                        res_total_all[i]["Low Bound"] == temp.iloc[k]["Low Bound"]) & (
                                                                 res_total_all[i]["High Bound"] == temp.iloc[k][
                                                             "High Bound"])]["AvgWinLoss"]))
            d = np.abs(metrics - np.median(metrics))
            mdev = np.median(d)
            s = d / mdev if mdev else 0.
            min_metric = metrics[s < 2]
            if min_metric.ndim == 2:
                min_metric = min_metric[0]
            min_metric = min(list(min_metric))
            temp.loc[k, "AvgWinLoss"] = min_metric
        res_total_all[i] = temp
        res_total_all[i] = res_total_all[i].sort_values(by="AvgWinLoss", ascending=False)
        res_total_all[i] = res_total_all[i].reset_index(drop=True)
    ###
    res_total = res_total_all
...
def SendMail(ticker, sortby, ImgFileNameList):
    ...
    s.sendmail('algo_notifications@acsysindia.com', ['algo_notifications@acsysindia.com', 'suprabhashsahu@acsysindia.com', 'aditya@shankar.biz'], msg.as_string())  #
    s.quit()
...
if __name__ == '__main__':
    ...
    #    PHASE 1 : Choosing different combinations of upper bound, lower bound and fisher lookback for different time periods and selecting the best 10 uncorrelated strategies(for each backtest length) and storing them.
    # ( ALL FUNCTIONS UNDER THIS PHASE HAS BEEN DEBUGGED AND CHECKED -11/1/2022 )
    ...
    res_test6 = select_all_strategies(72,dates, temp_og, ticker,save=True)
    # Here the selected strategies' results might be closely correlated with one another. To avoid redundant strategies giving same metrics, we weed out the correlated strategies.
    ss_test, res_test = select_strategies_from_corr_filter(res_test2,res_test4,res_test6, dates, temp_og, 1,10, ticker, save=True)
    ss_test, res_test = select_strategies_from_corr_filter(res_test2,res_test4,res_test6, dates, temp_og, 2,10, ticker, save=True)
    ss_test, res_test = select_strategies_from_corr_filter(res_test2,res_test4,res_test6, dates, temp_og, 3,10, ticker, save=True)


    #    PHASE 2 : Optimising the strategy weights. Here, the code gathers all the top strategies from Phase 1 and combines them to give a final signal optimized for a single metric   #
    # ( ALL FUNCTIONS UNDER THIS PHASE HAS BEEN DEBUGGED AND CHECKED -11/1/2022 )
    for number_of_optimization_periods in [1,2,3]: #1,2,3
        for recalib_months in [3,6,12]:  #3,6,12
            for num_strategies in [1,3, 5, 7]: #1,3,5,7
                for metric in ['rolling_sharpe', 'rolling_sortino', 'rolling_cagr', 'maxdrawup_by_maxdrawdown','outperformance']: #
                    ...
                    # Since the results at this stage would be for different 3-month periods (or 6 or 12), we will now combine it all to have signals for the WHOLE time period up until today
    ...
    #    PHASE 3 : At this stage all the resulting strategies are plotted and saved for visual aids #
    # ( ALL FUNCTIONS UNDER THIS PHASE HAS BEEN DEBUGGED AND CHECKED -11/1/2022 )
    res = []
    ...
    # PHASE 3b : All the results are then sorted according to the below metrics, zipped into a folder and then sent as an email #
    # ( ALL FUNCTIONS UNDER THIS PHASE HAS BEEN DEBUGGED AND CHECKED -11/1/2022 )
    ...
    zipf = zipfile.ZipFile(f'{ticker}.zip'.replace("^", ""), 'w', zipfile.ZIP_DEFLATED)
