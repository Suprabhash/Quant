...
from CacheUpdater import recalibrate_indices
...
def SendMail(subject_text_add, rebalance_recalibration_df, text_restricted,text_nifty, text_gold, text, printdf_nifty, printdf_gold, assets, ImgFileNameList, csv_file_path):
    ...
    msg['Cc'] = 'suprabhashsahu@acsysindia.com, aditya@shankar.biz, pratiksaxena@acsysindia.com, divakarank@acsysindia.com' #
    ...
    s.sendmail('algo_notifications@acsysindia.com', ['suprabhashsahu@acsysindia.com', 'algo_notifications@acsysindia.com', 'aditya@shankar.biz', 'pratiksaxena@acsysindia.com', 'divakarank@acsysindia.com'], msg.as_string())  #
    s.quit()
...
def execute_nifty(ticker, number_of_optimization_periods,recalib_months,num_strategies,metric):
    ...
    cwd = os.getcwd()
    print(f"This is the path before recalibration = {cwd}/{ticker}/SelectedStrategies/{ticker}_OptPeriods_{number_of_optimization_periods}_Selected_Strategies_ss.pkl")
    with open(
            f'/home/azurelinux/Desktop/LiveFiles/{ticker}/SelectedStrategies/{ticker}_OptPeriods_{number_of_optimization_periods}_Selected_Strategies_ss.pkl',
            'rb') as file:
        ss_test_imp = pickle.load(file)

    with open(
            f'/home/azurelinux/Desktop/LiveFiles/{ticker}/SelectedStrategies/{ticker}_OptPeriods_{number_of_optimization_periods}_Selected_Strategies_res.pkl',
            'rb') as file:
    ...
    if date_i_max >= len(ss_test_imp):
        ...
        cwd = os.getcwd()
        print(
            f"This is the path after recalibration = {cwd}/{ticker}/SelectedStrategies/{ticker}_OptPeriods_{number_of_optimization_periods}_Selected_Strategies_ss.pkl")
        print(f"Importing selected strategies: {datetime.now()}")
        with open(
                f'/home/azurelinux/Desktop/LiveFiles/{ticker}/SelectedStrategies/{ticker}_OptPeriods_{number_of_optimization_periods}_Selected_Strategies_ss.pkl',
                'rb') as file:
    ...
    with open(
            f'/home/azurelinux/Desktop/LiveFiles/{ticker}/weights/Results_Ticker{ticker}_LP{number_of_optimization_periods}_Recal{recalib_months}_NS{num_strategies}_M{metric}.pkl',
            'rb') as file:
...
def execute_gold(ticker, number_of_optimization_periods,recalib_months,num_strategies,metric):
    ...
    temp_res_gold.columns = ["Date"] + [column + "_gold" for column in temp_res_gold.columns if
                                        column != 'Date']
    temp_res.to_csv("Gold_Alpha_Nifty_Naive.csv")
        ...
        if ((datetime.now().hour==run_hour) & (datetime.now().minute==run_minute) & (datetime.now().second==00)) or running_for_the_first_time:
            if (datetime.today().isoweekday() < 6):
                ...
                if (pd.to_datetime(date.today()) in dates_recalibrating):
                    ...
                    with open(f'/nas/Algo/Live_emailer_updates/files_for_rebalance/BSE_RecalibPeriod_latest.pkl', 'rb') as file:
                ...
                temp_res.to_csv("Nifty50Naive.csv")

                azure_update_status_message = update_nifty_midcap_daily("Nifty50Naive.csv")
                azure_update_status_message = azure_update_status_message + update_gold_daily("Gold_Alpha_Nifty_Naive.csv")
                ...
                if running_for_the_first_time == True:
                    ...
                    SendMail(subject_text_add, pd.DataFrame(dates_recalibrate).set_index("Rebalancing Date").sort_values(by="Date", ascending=True), text_restricted,text_nifty, text_gold, text, printdf_nifty, printdf_gold, pd.concat([assetsb[-1][["Ticker", "Accelerating Momentum"]].set_index("Ticker"), pd.DataFrame([unit_ticker]).transpose().rename(columns={0:"Units"})], axis=1), ["Performance^NSEI.jpg", "PerformanceGOLDBEES.NS.jpg", "PerformanceNIFTY.jpg"], "Nifty50Naive.csv")
